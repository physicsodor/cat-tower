...
import type { Family, FamilyMap, BroDir } from "./family";

export const buildFamilyMap = <T extends Family>(
  list: ReadonlyArray<T>,
): FamilyMap => {
  const idx2kids = new Map<number, number[]>();
  const idx2info = new Map<number, import("./family").FamilyInfo>();
  for (const x of list) {
...
export const reparentKids = <T extends Family>(
  idx2family: FamilyMap,
  idx: number,
): { updater: (list: ReadonlyArray<T>) => T[] } => {
  const info = idx2family.get(idx);
  ...
  const updater = (list: ReadonlyArray<T>) =>
    list
      .filter((x) => x.idx !== idx)
      .map((x) =>
        x.mom === idx ? { ...x, mom, bro: idx2bro.get(x.idx) ?? "" } : x,
      );
  return { updater };
};

export const exitMom = <T extends Family>(
  idx2family: FamilyMap,
  targetSet: ReadonlySet<number>,
): { updater: (list: ReadonlyArray<T>) => T[] } => {
